...
[print(x) for x in local_dev if x.device_type == 'GPU']
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      layer_weights = tf.compat.v1.Variable(tf.compat.v1.truncated_normal([BOTTLENECK_TENSOR_SIZE, class_count], stddev=0.001), name='final_weights')
...
def main(_):
  ...
  if tf.compat.v1.gfile.Exists(FLAGS.summaries_dir):
    # tf.compat.v1.DeleteRecursively(FLAGS.summaries_dir)
    os.removedirs(FLAGS.summaries_dir)
  # tf.compat.v1.MakeDirs(FLAGS.summaries_dir)
